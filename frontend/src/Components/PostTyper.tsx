...
function PostTyper({
  ...
  const CreatePost = () => {
    (async () => {
      ...
      for (const file of files) {
        ...
        links += file.isVideo
          ? vid.data.link + ' '
          : (await UploadToImgurFile(file.file)).data.link + ' ';
      }
  ...
  const UploadImage = (e: any) => {
    ...
    target.innerHTML += ReactDOMServer.renderToStaticMarkup(
      isVideo ? <VideoEmbed url={link} /> : <ImageEmbed url={link} />,
    );
  };
    ...
    <div>
      ...
      <div className="post-typer-buttons">
        ...
        <button
          ref={sendButtonRef}
          onClick={CreatePost}
          className="post-typer-button button-field post-typer-sender"
        >
          Send
        </button>
      </div>
      {isEmojiPickerOpened ? (
        <EmojiPicker
          onEmojiClick={(emojiData: EmojiClickData, event: MouseEvent) => {
            textarea.current!.value += emojiData.isCustom
              ? `<:${emojiData.emoji}:> `
              : emojiData.emoji;
          }}
          customEmojis={
            self_user?.customization?.emojis
              ? self_user?.customization?.emojis
              : []
          }
          theme={Theme.DARK}
